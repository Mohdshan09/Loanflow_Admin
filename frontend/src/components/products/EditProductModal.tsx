...
const DEFAULT_FORM: CreateProductInput = {
  name: '',
  minAge: 21,
  maxAge: 60,
  minCreditScore: 700,
...
const productToForm = (product: Product): CreateProductInput => ({
  name: product.name,
...
const EditProductModal = ({ product, isOpen, onClose }: Props) => {
  ...
  const [formData, setFormData] = useState<CreateProductInput>(() =>
    product ? productToForm(product) : DEFAULT_FORM,
  );
  const [syncedKey, setSyncedKey] = useState<string | null>(
    isOpen && product ? product.id : null,
  );

  if (isOpen && product && syncedKey !== product.id) {
    setSyncedKey(product.id);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4">
      <div className="w-full max-w-2xl rounded-2xl bg-white shadow-2xl border border-slate-100 overflow-hidden">
        <div className="flex items-center justify-between border-b border-slate-100 bg-slate-50/50 px-6 py-4">
          <div>
            <h2 className="text-lg font-bold text-slate-900 leading-none">Edit Product</h2>

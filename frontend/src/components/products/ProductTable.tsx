...
const ProductTable = ({ products: propProducts }: ProductTableProps) => {
  ...
  const products: ProductType[] =
    propProducts ?? (data as { data?: ProductType[] } | undefined)?.data ?? [];
  ...
  if (isError && !propProducts) {
    return (
      <div className="rounded-xl border border-slate-200 bg-white p-12 text-center shadow-sm">
        <p className="text-sm font-semibold text-slate-900">Failed to load products</p>
        <p className="mt-1 text-sm text-slate-500">Something went wrong while fetching products.</p>
        <button
          type="button"
          onClick={() => refetch()}
  ...
  if (products.length === 0) {
    return (
      <div className="rounded-xl border border-slate-200 bg-white p-12 text-center shadow-sm">
        <h3 className="text-base font-semibold text-slate-900">No loan products yet</h3>
        <p className="mx-auto mt-1 max-w-md text-sm text-slate-500">
          Create your first loan product and define its eligibility criteria.
        </p>
      </div>
    );
  }
      ...
      <div className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
            ...
            <tbody className="divide-y divide-slate-100">
              {products.map((product) => (
                <tr key={product.id} className="transition-colors hover:bg-slate-50/60">
                  ...
                  <td className="px-5 py-3.5">
                    <AgeRange minAge={product.minAge} maxAge={product.maxAge} />
                  </td>

                  <td className="px-5 py-3.5">
                    <MinCreditScore score={product.minCreditScore} />
                  </td>

                  <td className="px-5 py-3.5">
                    <EmploymentTypes types={product.allowedEmploymentTypes} />
                  </td>

                  <td className="px-5 py-3.5">
                    <SalaryTypes types={product.allowedSalaryTypes} />
                  </td>
                  ...
                  <td className="px-5 py-3.5">
                    <CreatedDate date={String(product.createdAt)} />
                  </td>
        ...
        <div className="flex items-center justify-between border-t border-slate-100 bg-white px-5 py-3">
          <p className="text-xs text-slate-500">
            Showing <span className="font-semibold text-slate-700">1–{products.length}</span> of{' '}

...
const Topbar = () => {
    ...
    const products: Product[] =
        (productsData as { data?: Product[] } | undefined)?.data ?? [];
    const users: User[] =
    ...
    const hasResults =
        results.matchedProducts.length > 0 || results.matchedUsers.length > 0;
    ...
    const handleSelectUser = (_u: User) => {
        setIsOpen(false);
        setQuery("");
        navigate("/users");
    };
                    ...
                    <div className="absolute left-0 top-full mt-2 w-full rounded-xl border border-slate-200 bg-white shadow-2xl z-50 overflow-hidden">
                                                    ...
                                                    <div className="flex items-center gap-2.5">
                                                        ...
                                                        <div>
                                                            ...
                                                            <p className="mt-1 text-[10px] text-slate-400">
                                                                Min Score: {p.minCreditScore}+ · Min Salary: ₹{Number(p.minSalary).toLocaleString("en-IN")}
                                                            </p>
                                                        </div>
                                                    </div>
                                                    ...
                                                    >
                                                        <div className="flex items-center gap-2.5">
                                                            ...
                                                            <div>
                                                                ...
                                                                <p className="mt-1 text-[10px] text-slate-400">
                                                                    Credit: {u.creditScore} · ₹{Number(u.salary).toLocaleString("en-IN")}/mo
                                                                </p>
                                                            </div>
                                                        </div>
                        ...
                        <div className="border-t border-slate-100 bg-slate-50/70 px-3 py-2 text-right">
                            <span className="text-[10px] text-slate-400 font-medium">
                                Press <kbd className="rounded bg-white px-1 py-0.5 border border-slate-200 shadow-2xs font-mono text-[9px]">ESC</kbd> to close
                            </span>
                        </div>
                    </div>
                )}

...
const SystemStatusBar = ({
    ...
    // Determine actual live status (props take precedence if provided)
    let computedApi: SystemStatus = "loading";
    let computedDb: SystemStatus = "loading";

    if (propApiStatus && propDbStatus) {
        computedApi = propApiStatus;
        computedDb = propDbStatus;
    } else if (isPending) {
        computedApi = "loading";
        computedDb = "loading";
    } else if (isError || !data) {
        computedApi = "disconnected";
        computedDb = "disconnected";
    } else {
        computedApi = data.api === "connected" ? "connected" : "disconnected";
        computedDb =
    ...
    const isSystemOperational =
        computedApi === "connected" && computedDb === "connected";
    const isSystemLoading =
        computedApi === "loading" || computedDb === "loading";
        ...
        <div className="flex flex-wrap items-center gap-x-5 gap-y-2 rounded-xl border border-slate-200 bg-white px-5 py-3 shadow-sm transition-all">
            ...
            <div className="flex items-center gap-2">
                ...
                <div className="flex items-center gap-1.5">
                    ...
                    <span className={`text-xs font-semibold ${api.textClass}`}>
                        {api.label}
                    </span>
                </div>
            </div>
            ...
            <div className="flex items-center gap-2">
                ...
                <div className="flex items-center gap-1.5">
                    ...
                    <span className={`text-xs font-semibold ${db.textClass}`}>
                        {db.label}
                    </span>
                </div>
            </div>
